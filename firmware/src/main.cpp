...
bool calibrate_nvs_load(float H[3][3]) {
  ...
  H[0][0] = prefs.getFloat("h00", 1.0f);
  H[0][1] = prefs.getFloat("h01", 0.0f);
  H[0][2] = prefs.getFloat("h02", 0.0f);
  H[1][0] = prefs.getFloat("h10", 0.0f);
  H[1][1] = prefs.getFloat("h11", 1.0f);
  H[1][2] = prefs.getFloat("h12", 0.0f);
  H[2][0] = prefs.getFloat("h20", 0.0f);
  H[2][1] = prefs.getFloat("h21", 0.0f);
  H[2][2] = 1.0f;
  prefs.end();
  return true;
}
...
void calibrate_nvs_save(const float H[3][3]) {
  ...
  prefs.putFloat("h00", H[0][0]);
  prefs.putFloat("h01", H[0][1]);
  prefs.putFloat("h02", H[0][2]);
  prefs.putFloat("h10", H[1][0]);
  prefs.putFloat("h11", H[1][1]);
  prefs.putFloat("h12", H[1][2]);
  prefs.putFloat("h20", H[2][0]);
  prefs.putFloat("h21", H[2][1]);
...
void run_detection() {
  ...
  for (int i = 0; i < 4; i++) {
    DetectionResult r =
        color_detect_scan_pixels(imageMatrix, IMG_WIDTH, IMG_HEIGHT, colors[i]);
    if (r.found) {
      float mm_x, mm_y;
...
static void handle_calib_serial(String &line) {
  ...
  if (line.equalsIgnoreCase("SCAN")) {
    ...
    dot_count =
        calibrate_find_dots(imageMatrix, IMG_WIDTH, IMG_HEIGHT, dot_px, dot_py);
    scanned = true;
    solved = false;
    ...
    if (dot_count == 4) {
      ...
      Serial.println(
          "[CALIB] All 4 dots detected. Send SOLVE to compute homography.");
    } else {
  ...
  if (line.equalsIgnoreCase("REVIEW")) {
    if (!scanned) {
      Serial.println("[CALIB] No SCAN data.");
      return;
    }
  ...
  if (line.equalsIgnoreCase("SOLVE")) {
    ...
    if (dot_count < 4) {
      Serial.println("[CALIB] Not enough dots.");
      return;
    }
    ...
    Serial.printf("  [%7.4f  %7.4f  %7.4f]\n", calib_H[0][0], calib_H[0][1],
                  calib_H[0][2]);
    Serial.printf("  [%7.4f  %7.4f  %7.4f]\n", calib_H[1][0], calib_H[1][1],
                  calib_H[1][2]);
    Serial.printf("  [%7.4f  %7.4f  %7.4f]\n", calib_H[2][0], calib_H[2][1],
                  calib_H[2][2]);
  ...
  if (line.equalsIgnoreCase("SAVE")) {
    if (!solved) {
      Serial.println("[CALIB] Nothing to save.");
      return;
    }
...
static void dispatch_serial() {
  ...
  if (line.equalsIgnoreCase("CALIB") && !inCalibMode && !inServoTestMode) {
    ...
    Serial.println("[CALIB] Place 200x200mm paper with 4 dark dots centered "
                   "under robot base.");
    print_calib_help();
    return;
  }
  ...
  if (line.equalsIgnoreCase("SERVOTEST") && !inCalibMode && !inServoTestMode) {
    ...
    Serial.println(
        "[SERVO] Test mode active. Enter angles separated by spaces, e.g.:");
