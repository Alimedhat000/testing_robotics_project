                         ...
                         uint32_t out_uy[CALIB_N_POINTS])
{
    ...
    // Allocations → PSRAM, with a sane queue cap (100K entries is
    // plenty: max acceptable flood at HD is ~25 600 pixels).
    size_t npix = (size_t)w * h;
    size_t max_queue = npix < 100000 ? npix : 100000;
    
    uint8_t *visited = (uint8_t *)heap_caps_malloc(npix, MALLOC_CAP_SPIRAM);
    int *qmem = (int *)heap_caps_malloc(max_queue * 2 * sizeof(int), MALLOC_CAP_SPIRAM);
    if (!visited || !qmem) {
    ...
    for (int y = 0; y < h && nblobs < 32; y++) {
        for (int x = 0; x < w && nblobs < 32; x++) {
            ...
            int span;
            int avg_chroma = 0;
            int sz = flood_blob(pixels, w, h, x, y, visited, qx, qy, (int)max_queue,
    ...
    // Sort by contrast strength descending
...
static void gauss_elim(float A[N][N], float b[N]) {
  for (int col = 0; col < N; col++) {
    ...
    if (best != col) {
      for (int j = col; j < N; j++) { float t = A[col][j]; A[col][j] = A[best][j]; A[best][j] = t; }
...
bool calibrate_solve(const uint32_t ux[CALIB_N_POINTS],
  ...
  for (int i = 0; i < 4; i++) {
    ...
    A[2*i][0] = u; A[2*i][1] = v; A[2*i][2] = 1.0f; A[2*i][6] = -u*x; A[2*i][7] = -v*x; b[2*i] = x;
    A[2*i+1][3] = u; A[2*i+1][4] = v; A[2*i+1][5] = 1.0f; A[2*i+1][6] = -u*y; A[2*i+1][7] = -v*y; b[2*i+1] = y;
  }
  gauss_elim(A, b);
